...
public class Slave {
    ...
    public Slave(String masterHost, int masterPort, String slaveHost, int slavePort) {
...
//                  log.trace("Current stage not finished processing, nothing to write for now.");
    ...
    private void connectToMaster() throws IOException {
        ...
        if (this.slaveHost != null) {
            InetSocketAddress localSocketAddress = new InetSocketAddress(slaveHost, slavePort);
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length - 1; i++) {
            ...
            } else if (args[i].equals("-slave")) {
                String param = args[i + 1];
                if (param.contains(":")) {
                    slaveHost = param.substring(0, param.indexOf(":"));
                    try {
                        slavePort = Integer.parseInt(param.substring(param.indexOf(":") + 1));
                    } catch (NumberFormatException nfe) {
                        log.warn("Unable to parse port part of the master!  Failing!");
                        ShutDownHook.exit(10);
                    }
        ...
        Slave slave = new Slave(masterHost, masterPort, slaveHost, slavePort);
        slave.start();
    }
