...
public class TpccStressor extends AbstractCacheWrapperStressor implements Observer {
    ...
    private final List<Producer> producers = Collections.synchronizedList(new ArrayList<Producer>());
    private StatSampler statSampler;
    private final AtomicBoolean running = new AtomicBoolean(true);

    private final Timer finishBenchmarkTimer = new Timer("Finish-Benchmark-Timer");

    private final List<Stressor> stressors = Collections.synchronizedList(new LinkedList<Stressor>());
    ...
    private void createProducers(int arrivalRate) {
        ...
        if (!this.workloadGenerator.isOpenSystem()) {
            return;
        } // Closed System
        ...
        if (cacheWrapper.isPassiveReplication()) {
            ...
            if (cacheWrapper.isTheMaster()) {
                ...
                producerRates = new GroupProducerRateFactory(workloadGenerator.getRateDistribution(), arrivalRate * readWeight, cacheWrapper.getNumMembers() - 1,
    ...
    public Map<String, String> stress(CacheWrapper wrapper) {
        ...
        if (workloadGenerator.isOpenSystem()) {
            ...
            //    createProducers(workloadGenerator.getMaxArrivalRate());
            //}
        }
        ...
        log.info("Before processResults");
