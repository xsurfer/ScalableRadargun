...
//            int newSize = ElasticMaster.getInstance().masterConfig.getSlaveCount() + newSlaves.size();
//            //log.debug("Editing slaves number on masterConfig: from " + ElasticMaster.getInstance().masterConfig.getSlaveCount() + " to " + newSize);
//            //ElasticMaster.getInstance().masterConfig.setSlavesCount(newSize);
//
//            log.debug("Editing slaves number on the current stage: from " + ElasticMaster.getInstance().state.getCurrentDistStage() + " to " + newSize);
//            ElasticMaster.getInstance().state.getCurrentDistStage().setActiveSlavesCount(newSize);
//
//            log.debug("Editing size for the current fixed benchmark");
//            ElasticMaster.getInstance().state.getCurrentBenchmark().currentFixedBenchmark().setSize(newSize);
//
//            prepareNextStage();
...
//        DistStage toExecute = ElasticMaster.getInstance().state.getNextDistScalingStageToProcess();
//        if (toExecute == null) {
//            ShutDownHook.exit(0);
//            //return;
//        }
...
//        ElasticMaster.getInstance().writeBufferMap.clear();
//
...
//             *  SocketChannel slave = ElasticMaster.getInstance().slaves.get( ElasticMaster.getInstance().slave2Index.get(newSlaves.get().get(i)) );
//             *
...
//            toSerialize.initOnMaster(ElasticMaster.getInstance().state, ElasticMaster.getInstance().slave2Index.get(slave));
...
//            ElasticMaster.getInstance().writeBufferMap.put(slave, ByteBuffer.wrap(bytes));
//        }
//    }
//
...
//                for (int i = 0; i < newSlaves.size(); i++) {
//                    SocketChannel slave = newSlaves.get(i);
//                    ElasticMaster.getInstance().slavesReadyToMerge.add(slave);
//                }
//                ElasticMaster.getInstance().communicationSelector.wakeup();
//                break;
//            }
//        }
//    }
...
//
//        ByteBuffer byteBuffer = ElasticMaster.getInstance().readBufferMap.get(socketChannel);
...
//
//        if (value == -1) {
//            log.warn("Slave stopped! Index: " + ElasticMaster.getInstance().slave2Index.get(socketChannel) + ". Remote socket is: " + socketChannel);
//            key.cancel();
//            if (!ElasticMaster.getInstance().slaves.remove(socketChannel)) {
//                throw new IllegalStateException("Socket " + socketChannel + " should have been there!");
//            }
//            ElasticMaster.getInstance().releaseResourcesAndExit();
...
//                ElasticMaster.getInstance().readBufferMap.put(socketChannel, replacer);
//                if (log.isTraceEnabled())
...
//                log.debug(ElasticMaster.getInstance().state.getCurrentDistScalingStage().getClass().getName());
//                if (ack.getStageName().compareTo(ElasticMaster.getInstance().state.getCurrentDistScalingStage().getClass().getName()) == 0) {
//                    responses.add(ack);
...
//            if (!ElasticMaster.getInstance().state.distScalingStageFinished(responses)) {
//                log.error("Exiting because issues processing current stage: " + ElasticMaster.getInstance().state.getCurrentDistStage());
//                ElasticMaster.getInstance().releaseResourcesAndExit();
//            }
//
//            prepareNextStage();
...
//            //int ackLeft = ElasticMaster.getInstance().state.getSlavesCountForCurrentScalingStage() - responses.size();
...
//        SocketChannel socketChannel = (SocketChannel) key.channel();
//        ByteBuffer buf = ElasticMaster.getInstance().writeBufferMap.get(socketChannel);
//        if (log.isTraceEnabled())
...
//
//            //log.info(ElasticMaster.getInstance().state.getCurrentDistScalingStage().getClass().getName());
//            // org.radargun.mandatoryStages.WebSessionBenchmarkStage
//
////            if(ElasticMaster.getInstance().state.getCurrentDistScalingStage().getClass().getName().contains("TpccBenchmarkStage")){
//
//            if(ElasticMaster.getInstance().state.getCurrentDistStage().getId().equals(ElasticMaster.getInstance().state.getCurrentDistScalingStage().getId())){
...
//        //if (processedSlaves == ElasticMaster.getInstance().state.getSlavesCountForCurrentScalingStage()) {
//        if (processedSlaves == this.newSlaves.size()) {
//            log.trace("Successfully completed broadcasting stage " + ElasticMaster.getInstance().state.getCurrentDistScalingStage());
//            processedSlaves = 0;
//            ElasticMaster.getInstance().writeBufferMap.clear();
//            responses.clear();
//        }
