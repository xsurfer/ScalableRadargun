...
// * ElasticMaster.getInstance(null);
// * <p/>
// * Date: 11/30/12
// * Time: 3:24 PM
...
//        this.slaves = slaves;
//        this.communicationSelector = communicationSelector;
...
//            log.debug("Ending ClusterExecutor");
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//    }
...
//        log.debug("Starting prepareNextStage");
...
//            SocketChannel slave = ElasticMaster.getInstance().slaves.get(i);
//            slave.configureBlocking(false);
//            slave.register(ElasticMaster.getInstance().communicationSelector, SelectionKey.OP_WRITE);
//
//            toSerialize = currentStage.clone();
//            toSerialize.initOnMaster(ElasticMaster.getInstance().state, i);
...
//        while (!ElasticMaster.getInstance().stopped) {
//            ElasticMaster.getInstance().communicationSelector.select();
//            /** Registro nuovi slave nel caso slavesReadyToMerge non è vuota **/
//            if(!ElasticMaster.getInstance().slavesReadyToMerge.isEmpty()){
//                for (SocketChannel sc : ElasticMaster.getInstance().slavesReadyToMerge) {
//                    sc.configureBlocking(false);
//                    sc.register(ElasticMaster.getInstance().communicationSelector, SelectionKey.OP_READ);
//                }
//                ElasticMaster.getInstance().slavesReadyToMerge.clear();
//                continue;
//            }
//
//            Set<SelectionKey> keys = ElasticMaster.getInstance().communicationSelector.selectedKeys();
...
//                log.info(ElasticMaster.getInstance().state.getCurrentDistStage().getClass().getName());
//                if (ack.getStageName().compareTo(ElasticMaster.getInstance().state.getCurrentDistStage().getClass().getName()) == 0) {
...
//                }
//                int ackLeft = ElasticMaster.getInstance(null).state.getSlavesCountForCurrentStage() - responses.size();
//                log.info( ackLeft + " ACKs left" );
//            }
//        }
//
//        if (responses.size() == ElasticMaster.getInstance(null).state.getSlavesCountForCurrentStage()) {
//            if (!ElasticMaster.getInstance(null).state.distStageFinished(responses)) {
//                log.error("Exiting because issues processing current stage: " + ElasticMaster.getInstance(null).state.getCurrentDistStage());
//                ElasticMaster.getInstance(null).releaseResourcesAndExit();
//            }
//
//            prepareNextStage();
//        }
...
//        SocketChannel socketChannel = (SocketChannel) key.channel();
//        ByteBuffer buf = ElasticMaster.getInstance(null).writeBufferMap.get(socketChannel);
//        if (log.isTraceEnabled())
...
//        }
//        if (processedSlaves == ElasticMaster.getInstance(null).state.getSlavesCountForCurrentStage()) {
//            log.trace("Successfully completed broadcasting stage " + ElasticMaster.getInstance(null).state.getCurrentDistStage());
//            processedSlaves = 0;
//            ElasticMaster.getInstance(null).writeBufferMap.clear();
//            responses.clear();
//        }
//    }
//}
