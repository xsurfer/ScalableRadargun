...
public abstract class AbstractDistStage implements DistStage {
   ...
   /**
    * Just in case of error, pasted during merging by Fabio +    protected void logDurationInfo(List<DistStageAck> acks)
    * { +        if (!log.isInfoEnabled()) return; + +        Map<Integer, String> data = new TreeMap<Integer,
    * String>();  // make sure this is sorted +        for (DistStageAck dsa : acks){ +
    * data.put(dsa.getSlaveIndex(), Utils.prettyPrintTime(dsa.getDuration())); +        } + +        String
    * processingDuration = "Durations ["; +        boolean first = true; +        for (Map.Entry<Integer, String> e :
    * data.entrySet()) { +            if (first) first = false; +            else processingDuration += ", "; +
    *   processingDuration += e.getKey() + ":" + e.getValue(); +        } +        log.info(Thread.currentThread().getName()
    * + " - " + getClass().getSimpleName() + " received ack from all (" + acks.size() + ") slaves. " +
    * processingDuration + "]" ); +    }
    */
