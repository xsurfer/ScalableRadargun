...
/**
 * Created with IntelliJ IDEA. User: Fabio Perfetti Date: 3/18/13 Time: 5:10 PM
 */
public abstract class AbstractWorkloadGenerator extends Observable implements Cloneable, Serializable {
   ...
   public void setRateDistribution(String rateDistribution) {
      this.rateDistribution = RateDistribution.valueOf(rateDistribution.toUpperCase());
   }
   ...
   /**
    * Returns the arrival rate.<br/> If maxArrivalRate setted and getCurrentArrivalRate()>maxArrivalRate then
    * maxArrivalRate will be returned
    *
    * @return maxArrivalRate if getCurrentArrivalRate() > maxArrivalRate && maxArrivalRate > 0<br/> else
   ...
   private class TimeIncrementer implements Runnable {
      ...
      public TimeIncrementer(BlockingQueue<Double> queue) {
         this.queue = queue;
      }
